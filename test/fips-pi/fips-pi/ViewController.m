 ...
 * @param testDescription descriptioni of test
 */
...
/*!
 * @brief Performs test case pass/fail
 * @param result (boolean) Condition being tested
 * @param testDescription descriptioni of test
 */
...
/*!
 * @brief Displays an error message on the UI
...
//    [m_calculatedLabel setText:f1];     // TODO remove this when compiler issue is fixed! - Note, this affects simulatoer only
    ...
    while(true) {
        ...
        // ------------------------------------------
        testDescription = startTest(@"Test 1.0 Confirm FIP mode turned on");
        ...
        testDescription = startTest(@"Test 1.1 Confirm FIP mode turned off");
        retVal = FIPS_mode_set(0 /*off*/);
        fipsMode = FIPS_mode();
        assertT2Test(retVal == T2True, testDescription );
        assertT2Test(fipsMode == T2FipsModeOff, testDescription );
        
        testDescription = startTest(@"Test 1.2 Confirm FIP mode turned on");
        retVal = FIPS_mode_set(1 /*on*/);
        fipsMode = FIPS_mode();
        assertT2Test(retVal == T2True, testDescription );
        assertT2Test(fipsMode == T2FipsModeOn, testDescription );
    ...
    //    NSLog(@"INFO: Recalled value: \"%@\" with key \"%@\" ",recalledValue, key2);
        ...
        if (iterations++ >= NUM_ITERATIONS_TO_PERFORM)
            break;
    ...
    // ----------------
    // Place any tests here that only should run once (regardless of what NUM_ITERATIONS_TO_PERFORM says
    // ----------------
    ...
    if (result == 0) {
        ...
        if (result == 0) {
            ...
            [player play];
            assertT2Test(TRUE, testDescription );
        } else {
...
void testSqlCipher() {
    ...
    NSLog(@"DB located at %@", databasePath);
    
    NSFileManager *filemgr = [NSFileManager defaultManager];
    
    
    // Create the database file if not alredy there
    ...
    // Recall the contact from the database
    if (sqlite3_open(dbpath, &contactDB) == SQLITE_OK) {
